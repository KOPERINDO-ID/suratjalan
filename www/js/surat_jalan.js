		...
		success: function (data) {
			...
			$.each(data.data.data, function (i2, item2) {
				...
				if (item2.karyawan_id == 13) {
					...
					penjualan_value += '</td>';
					penjualan_value += '<td align="left" data-popup=".detail-sales-surat-jalan" onclick="detailPenjualanProduksiSj(\'' + item2.penjualan_id + '\')" style="' + bbgselesaiprodsj + ' border-bottom:1px solid gray; border-left:1px solid gray;"  class="label-cell popup-open"  ><font style="color:white;" class="text-add-colour-black-soft"  >' + moment(item2.penjualan_tanggal).format('DDMMYY') + '-' + item2.penjualan_id.replace(/\INV_/g, '').replace(/^0+/, '') + '</font></td>';
